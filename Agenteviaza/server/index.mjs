...
function createAgent(){
  ...
  async function resolveIATATool(args){ try { console.log('🔧 resolveIATA chamada com:', args); let { origem, destino } = args; if (args?.context?.origem) origem = args.context.origem; if (args?.context?.destino) destino = args.context.destino; console.log('📥 Parâmetros extraídos:', { origem, destino }); const { origem: o, destino: d } = ResolveIATASchema.parse({ origem, destino }); const dep=resolveIATA(o); const des=resolveIATA(d); console.log('✅ resolveIATA retornando:', { dep, des }); return { dep, des }; } catch (err) { console.error('❌ resolveIATA ERRO:', err.message); throw err; } }
  async function parseDatesTool(args){ try { console.log('🔧 parseDates chamada com:', args); let { ida, volta } = args; if (args?.context?.ida) ida = args.context.ida; if (args?.context?.volta) volta = args.context.volta; console.log('📥 Parâmetros extraídos:', { ida, volta }); const { ida: i, volta: v } = DateParseSchema.parse({ ida, volta }); const dpt=parseNaturalDate(i); const dst=v?parseNaturalDate(v):null; if(!dpt) throw new Error('Data de ida inválida'); if(!withinWindow(dpt)) throw new Error('Data de ida fora da janela de 360 dias'); if(dst){ if(!withinWindow(dst)) throw new Error('Data de volta fora da janela de 360 dias'); if(new Date(dst) < new Date(dpt)) throw new Error('Volta não pode ser antes da ida'); } console.log('✅ parseDates retornando:', { dpt, dst, ow: !dst }); return { dpt, dst, ow: !dst }; } catch (err) { console.error('❌ parseDates ERRO:', err.message); throw err; } }
  async function parsePassengersTool(args){ try { console.log('🔧 parsePassengers chamada com:', args); let { total, composicao } = args; if (args?.context?.total) total = args.context.total; if (args?.context?.composicao) composicao = args.context.composicao; console.log('📥 Parâmetros extraídos:', { total, composicao }); const { total: t, composicao: c } = PaxParseSchema.parse({ total, composicao }); const totalNum = t!=null ? (typeof t==='number'? t : parseNumberPt(t)) : undefined; let chd=0,bby=0; if(c){ const s=stripDiacritics(c.toLowerCase()); const mChd=s.match(/(\d+|uma|um|duas|dois|tres|três|quatro|cinco|seis|sete|oito|nove)\s*crianc/); const mBby=s.match(/(\d+|uma|um|duas|dois|tres|três|quatro|cinco|seis|sete|oito|nove)\s*beb/); if(mChd) chd=parseNumberPt(mChd[1])??0; if(mBby) bby=parseNumberPt(mBby[1])??0; } const totalFinal = totalNum ?? (chd + bby + 1); const adt=Math.max(totalFinal - chd - bby, 0); if(adt+chd+bby>9) throw new Error('Máximo 9 passageiros por cotação'); console.log('✅ parsePassengers retornando:', { adt, chd, bby }); return { adt, chd, bby }; } catch (err) { console.error('❌ parsePassengers ERRO:', err.message); throw err; } }
  async function buildQuoteLinkTool(args){ try { console.log('🔧 buildQuoteLink chamada com:', args); let stateArgs = args; if (args?.context) stateArgs = args.context; console.log('📥 State extraído:', stateArgs); const result = await buildQuoteLinkStandalone(stateArgs); console.log('✅ buildQuoteLink retornando:', result); return result; } catch (err) { console.error('❌ buildQuoteLink ERRO:', err.message); throw err; } }

  const agent = new Agent({ name:'Viaza Assistant', instructions, ...(OPENAI_MODEL ? { model: OPENAI_MODEL } : {}), tools:[ {type:'function', name:'resolveIATA', parameters:ResolveIATAJson, invoke:resolveIATATool, needsApproval:()=>false}, {type:'function', name:'parseDates', parameters:DateParseJson, invoke:parseDatesTool, needsApproval:()=>false}, {type:'function', name:'parsePassengers', parameters:PaxParseJson, invoke:parsePassengersTool, needsApproval:()=>false}, {type:'function', name:'buildQuoteLink', parameters:StateJson, invoke:buildQuoteLinkTool, needsApproval:()=>false} ] });
  return agent;
}
