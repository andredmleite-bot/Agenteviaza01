...
function within365(iso){ const today=new Date(); const target=new Date(iso); const diff=Math.ceil((target-today)/(1000*60*60*24)); return diff>=0 && diff<=365; }
...
async function buildQuoteLinkStandalone(stateArgs){ const s=StateSchema.parse(stateArgs); const dep=s.dep.toUpperCase(); const des=s.des.toUpperCase(); if(!OFFICIAL_IATA.has(dep)||!OFFICIAL_IATA.has(des)) throw new Error('IATA inválido fora da lista oficial'); if(dep===des) throw new Error('Origem e destino não podem ser iguais'); if(!within365(s.dpt)) throw new Error('Data de ida fora da janela de 365 dias'); let dstVal=s.dst; if(s.ow){ dstVal=null; } else { if(!dstVal) throw new Error('Ida e volta requer dst válida'); if(!within365(dstVal)) throw new Error('Data de volta fora da janela de 365 dias'); if(new Date(dstVal) < new Date(s.dpt)) throw new Error('Volta não pode ser antes da ida'); } const state={ dep, des, adt:s.adt, chd:s.chd, bby:s.bby, dpt:s.dpt, dst:dstVal, ec:true, ow:s.ow }; const encoded=encodeURIComponent(JSON.stringify(state)); return `https://gomilhas.com.br/app/flights?state=${encoded}`; }

function createAgent(){
  const instructions = `Você é o assistente da Gomilhas. Entenda pedidos de viagem, colete origem/destino, datas (ida/volta) e passageiros (ADT/CHD/BBY ≤9). Valide IATA, datas futuras em até 365 dias, ida ≤ volta. Não usamos milhas do cliente; emitimos por milhas do nosso banco ou convencional, buscando a melhor tarifa. Quando confirmado, gere a URL https://gomilhas.com.br/app/flights?state=<JSON_URL_ENCODED> com state {dep,des,adt,chd,bby,dpt,dst|null,ec=true,ow}. Responda curto, claro e amigável.`;
  ...
  async function parseDatesTool(args){ const { ida, volta } = DateParseSchema.parse(args); const dpt=parseNaturalDate(ida); const dst=volta?parseNaturalDate(volta):null; if(!dpt) throw new Error('Data de ida inválida'); if(!within365(dpt)) throw new Error('Data de ida fora da janela de 365 dias'); if(dst){ if(!within365(dst)) throw new Error('Data de volta fora da janela de 365 dias'); if(new Date(dst) < new Date(dpt)) throw new Error('Volta não pode ser antes da ida'); } return { dpt, dst, ow: !dst }; }
...
app.get('/', (req,res)=>{ res.json({ name:'Gomilhas Agent API', message:'Use POST /api/chat para interagir com o agente.', health:'/api/health', chat:'/api/chat' }); });
app.get('/api/health',(req,res)=>{ res.json({ ok:true }); });

app.post('/api/chat', async (req,res)=>{ const { sessionId, message } = req.body||{}; if(!sessionId||!message) return res.status(400).send('Parâmetros obrigatórios'); const result = await (async ()=>{ if(isConfirmation(message) && pendingQuotes.has(sessionId)){ try{ const url = await buildQuoteLinkStandalone(pendingQuotes.get(sessionId)); pendingQuotes.delete(sessionId); return { reply:`Pronto! Aqui está sua cotação:\n${url}` }; } catch{ pendingQuotes.delete(sessionId); return { reply:'Erro ao gerar link após confirmação. Vamos ajustar dados e tentar novamente?' }; } } if(!OPENAI_API_KEY){ return { reply:'Backend sem OPENAI_API_KEY configurada.' }; } const hist = [ { role:'user', content: message } ]; const prompt = hist.map(m=>`${m.role==='user'?'Cliente':'Assistente'}: ${m.content}`).join('\n') + '\nAssistente:'; try{ const result = await run(agent, prompt); let reply = result?.finalOutput ?? 'Não consegui gerar resposta.'; return { reply }; } catch{ return { reply:'Instabilidade no agente. Tente novamente.' }; } })(); return res.json(result); });
