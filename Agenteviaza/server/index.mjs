...
function createAgent(){
  ...
  async function resolveIATATool(args){ try { console.log('🔧 resolveIATA chamada com:', args); const { origem, destino } = ResolveIATASchema.parse(args); const dep=resolveIATA(origem); const des=resolveIATA(destino); console.log('✅ resolveIATA retornando:', { dep, des }); return { dep, des }; } catch (err) { console.error('❌ resolveIATA ERRO:', err.message); throw err; } }
  async function parseDatesTool(args){ try { console.log('🔧 parseDates chamada com:', args); const { ida, volta } = DateParseSchema.parse(args); const dpt=parseNaturalDate(ida); const dst=volta?parseNaturalDate(volta):null; if(!dpt) throw new Error('Data de ida inválida'); if(!withinWindow(dpt)) throw new Error('Data de ida fora da janela de 360 dias'); if(dst){ if(!withinWindow(dst)) throw new Error('Data de volta fora da janela de 360 dias'); if(new Date(dst) < new Date(dpt)) throw new Error('Volta não pode ser antes da ida'); } console.log('✅ parseDates retornando:', { dpt, dst, ow: !dst }); return { dpt, dst, ow: !dst }; } catch (err) { console.error('❌ parseDates ERRO:', err.message); throw err; } }
  async function parsePassengersTool(args){ try { console.log('🔧 parsePassengers chamada com:', args); const { total, composicao } = PaxParseSchema.parse(args); const totalNum = total!=null ? (typeof total==='number'? total : parseNumberPt(total)) : undefined; let chd=0,bby=0; if(composicao){ const s=stripDiacritics(composicao.toLowerCase()); const mChd=s.match(/(\d+|uma|um|duas|dois|tres|três|quatro|cinco|seis|sete|oito|nove)\s*crianc/); const mBby=s.match(/(\d+|uma|um|duas|dois|tres|três|quatro|cinco|seis|sete|oito|nove)\s*beb/); if(mChd) chd=parseNumberPt(mChd[1])??0; if(mBby) bby=parseNumberPt(mBby[1])??0; } const totalFinal = totalNum ?? (chd + bby + 1); const adt=Math.max(totalFinal - chd - bby, 0); if(adt+chd+bby>9) throw new Error('Máximo 9 passageiros por cotação'); console.log('✅ parsePassengers retornando:', { adt, chd, bby }); return { adt, chd, bby }; } catch (err) { console.error('❌ parsePassengers ERRO:', err.message); throw err; } }
  async function buildQuoteLinkTool(stateArgs){ try { console.log('🔧 buildQuoteLink chamada com:', stateArgs); const result = await buildQuoteLinkStandalone(stateArgs); console.log('✅ buildQuoteLink retornando:', result); return result; } catch (err) { console.error('❌ buildQuoteLink ERRO:', err.message); throw err; } }
